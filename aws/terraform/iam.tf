...
# Custom Policy for ECS Execution Role (for accessing SSM parameters)
resource "aws_iam_role_policy" "ecs_execution_role_custom" {
  name = "${var.project_name}-ecs-execution-role-custom"
...
# Custom Policy for ECS Task Role (for S3 access)
    ...
    Statement = [
      {
        ...
        Resource = [
          "${aws_s3_bucket.anigmaa_uploads.arn}/*"
        ]
      },
      {
        Effect = "Allow"
...
# Custom Policy for ECS Task Role (for CloudWatch logs)
resource "aws_iam_role_policy" "ecs_task_role_cloudwatch" {
  name = "${var.project_name}-ecs-task-role-cloudwatch"
  role = aws_iam_role.ecs_task_role.id
...
# Custom Policy for ECS Task Role (for ECS Exec)
resource "aws_iam_role_policy" "ecs_task_role_exec" {
  name = "${var.project_name}-ecs-task-role-exec"
  role = aws_iam_role.ecs_task_role.id
...
# Lambda Execution Role Policy Attachment - Basic VPC Execution
...
# Lambda Policy for SSM Parameters (for secrets)
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${var.project_name}-backend:*"
        ]
      }
    ]
  })
