...
**Note**: Returns all events user is involved with (hosted or joined)
...
**Note**: Returns only events created by the current user
...
**Note**: Returns only events the user has joined as attendee
...
**Note**: For getting event Q&A and asking questions, see [Event Endpoints - Q&A section](#get-event-qa)
...
- Auth registration and login (`/api/v1/auth/register`, `/api/v1/auth/login`, etc.)
- Event listing and details (`GET /api/v1/events`, `GET /api/v1/events/:id`)
- Profile viewing (`/api/v1/profile/:username`)
...
### Host-Only Routes
Some endpoints are restricted to event hosts only:
